...
export default function request(url, option) {
  ...
  if (newOptions.body) {
    ...
    let bodyJson = url.indexOf('updateFile') == -1 ? JSON.parse(newOptions.body) : newOptions.body
    for (var key in bodyJson) {
      formData.append(key, !isNull(bodyJson[key]) ? bodyJson[key] : '');
    }
    if (newOptions.method && newOptions.method == 'post') {
